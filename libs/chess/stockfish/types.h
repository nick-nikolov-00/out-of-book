...
enum class RatingBucket : uint8_t {
    UNDER_1000 = 0,
    R1000_1199,
    R1200_1399,
    R1400_1599,
...
constexpr size_t NUMBER_OF_BUCKETS = static_cast<size_t>(RatingBucket::NUM_BUCKETS);

constexpr RatingBucket ratingBucket(int rating) {
    if (rating < 1000)
        return RatingBucket::UNDER_1000;

    if (rating >= 2400)
        return RatingBucket::R2400_PLUS;

    return static_cast<RatingBucket>(1 + (rating - 1000) / 200);
}
...
class Move {
   public:
    ...
    // we aggregated all children in this bucket cause we can't trust the move distribution (too few visits)
    // or the child was dropped due to too few visits
    static constexpr Move forcedAggregation() { return Move(0xFFFF); }
    // game ended here
    static constexpr Move termination() { return Move(130); }
